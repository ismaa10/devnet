...
ACCESS_TOKEN = 'ZTI0MDlhZDktODYwMi00ZDU3LTg2YmQtZDk1YTU4YWZlYWExZWU1YTBjMGItNDNj'  # Replace None with your access token. Shroud with quotes.
ROOM_NAME = 'DevNet Express - Buenos Aires'  # Replace None with the name of the room to be created. Shroud with quotes.
YOUR_MESSAGE = '\@MissionBot mision cumplida'  # Replace None with the message that you will post to the room. Shroud with quotes.
...
def findRoom(the_header, room_name):
    ...
    for room in resp["items"]:
        if room["title"] == room_name:
            ...
            print(
                "MISSION: findRoom: REPLACE None WITH CODE THAT PARSES JSON TO ASSIGN ROOM ID VALUE TO VARIABLE roomId")
            roomId = room['id']
            print('Room_id devuelto por la funcion find_room: ' + roomId)
            break
    return (roomId)
...
def createRoom(the_header, room_name):
    ...
    if roomId == None:
        ...
        print(
            "MISSION: createRoom: REPLACE None WITH CODE THAT PARSES JSON TO ASSIGN ROOM ID VALUE TO VARIABLE roomId.")
        roomId = var['id']
    return (roomId)
...
def getRoomInfo(the_header, roomId):
    ...
    print('Nombre de la sala: ' + str(resp['title']))
